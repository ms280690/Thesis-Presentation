...
In conclusion, we  explored the various approaches for bringing languages together. We looked at the state and support for embedded domain specific languages in \progLang{Haskell}. Specifically, we looked at \progLang{Prolog} in 
\progLang{Haskell}. Our ideas were tested by the means of some prototypes to find out that \progLang{Haskell} is an effective tool for embedding.  Lastly, we contributed towards solving the programmer's dilemma of choosing programming languages.
