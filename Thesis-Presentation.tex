...
\note[item]{Choosing a language is quite a daunting task. There are costs related to migration, training among others.}
\note[item]{And not to mention the options.}
\begin{figure}
...
\item \progLang{Prolog} implementations in \progLang{Haskell}.

\item Libraries to replicate logic programming functionalities.
...
note[item]{In this section we will talk about, the contributions which are not neccessary an improvement on the current work but a new
...
\item Multi-stage interpretation strategy.
\note[item]{Because of this we can have a multi stage interpretation strategy.}
\end{itemize}
\end{frame}
