...
\begin{document}
	...
	\begin{frame}
		...
		\note[item]{Good afternoon everybody. My name is Mehul Solanki. And today I would like to talk about embedding \progLang{Prolog} in \progLang{Haskell}.}
	\end{frame}
...
\note[item]{The problem this thesis contributes to and the existing work.}
...
\begin{itemize}
\note[item]{For the background, we will talk about,}
...
\note[item]{How they are classified into paradigms depending on their characteristics.}

\item Functional and logic programming.
\note[item]{the declarative stlye of programming and it's sub-paradigms functional and logic programming in particular,} 
...
\note[item]{and is purely functional meaning every function in Haskell is a function in the mathematical sense (i.e., "pure").}
\end{itemize}
...
\begin{itemize}
\item Declarative style of programming, describes (declaratively) what to do and not (operationally) how to do it. 
...
\note[item]{A functional language is all about building and applying functions.}

\item In logic programming, a program consists of a collection of statements expressed as formulas in symbolic logic. 
...
\note[item]{There are rules of inference from logic that allow a new formula to be derived from old ones, with the guarantee that if the old formulas are true, so is the new one.}
...
\begin{itemize}
\item \progLang{Prolog} has been embedded in host languages such as \progLang{Java, Lisp, Scheme}.
...
\end{itemize}
 \note[item]{None of these are currently under active development and provide limited functionality. }
...
%% -------------------------------------------------------------------
\begin{frame}{Merging Paradigms}
\note[item]{In this section we talk about languages that exhibit properties from multiple paradigms.}
\begin{itemize}
\item
  Merging paradigms means combining different programming paradigms or
  programming styles into a single environment resulting in a hybrid
  language. 
...
\item Functional Logic programming languages include:
...
\item For example, "Clarissa", a voice user interface for browsing procedures on the International Space Station.
	\note[item]{For instance, \progLang{Prolog} is often seen as a special purpose language, being effective for rule based systems. For this reason it was chosen to implement "Clarissa", a voice user interface for browsing space station procedures is written in SICStus
...
\note[item]{One option is to replicate the target language functionality in the current environment, and that is what we will look at 
...
\begin{frame}
   ...
   \begin{block}{Thesis statement}
     ...
     embedding the latter into the former and analysing the result of
     doing so\dots\,.
     \note[item]{\textcolor{red}{Read out Thesis statement.}}
   \end{block}
        \begin{itemize}
      \item Replicating functionality.
      \note[item]{One option is to replicate the target language functionality in the current environment,}
...
\item Monadic computations take place in a ``bubble.''
...
and hence the computation remains pure.}
...
\begin{itemize}
\item Pattern matching is a feature of \progLang{Haskell} (and other
  similar languages).
...
\note[item]{Let us take a look at a few examples. In these examples like in \progLang{Prolog} variables are capital letters.}
\begin{itemize}
...
Y = 2.  
\end{minted}
\note[item]{In the first example, what values must \prologConstruct{X} and \prologConstruct{Y} have for the left and right hand side to be equal.}
\note[item]{After unification, \prologConstruct{X} is instantiated to 1 and \prologConstruct{Y} to 2.}
...
Y = t(k) 
\end{minted} 
\note[item]{The second example looks complicated but is essentially the same as example 1.}
...
%% -------------------------------------------------------------------
\begin{frame}{Overview of what we did }
	
	\frametitle{Overview of what we did }
\begin{itemize}
...
\note[item]{We will now talk about elements of Prototype 1}
%% -------------------------------------------------------------------
\begin{frame}[fragile=singleslide]
    \frametitle{{Functorizing a language}}
    \begin{itemize}
    \item Traditional abstract grammars are recursive and closed.
    \item Open languages let other tools add, for instance,
      ...
      \item Closed language:
        \begin{minted}[linenos,gobble=8]{haskell}
          data Term = Struct Atom [Term]
                    | Var VariableName
                    | Wildcard
                    | Cut Int deriving (Eq, Data, Typeable)
        \end{minted}
        \note[item]{Because \haskellConstruct{Term} is on the left and right hand side, the grammar is data type.}
      ...
      \item Open language:
        ...
        \note[item]{Note, that \haskellConstruct{FlatTerm} does not occur on the right hand side.}
    \end{itemize}
...
\begin{frame}
	...
	\note[item]{Here, we make use of the \codeLibrary{unification-fd} library. This is a general purpose library which we adapt for our prototypes. It provides highly efficient algorithms for unification in \progLang{Haskell} by hiding imperative calculations in a monad.}
	...
	\begin{itemize}
		...
		\note[item]{the language varaibles are converted to state varaibles which store, pass and mutate values.}
...
\note[item]{We will now talk about elements of Prototype 2}
\begin{frame}
...
\note[item]{We will now talk about elements of Prototype 3}
\begin{frame}
\frametitle{Contributions}
...
\note[item]{We will now talk about elements of Prototype 4}
\begin{frame}
\frametitle{Contributions}
...
\note[item]{one reason why this can be done is that functions in \progLang{Haskell} are first class citizens; they are data.}
...
In conclusion,
\begin{itemize}
\item we explored the various approaches for bringing languages together. 

\item We looked at the state and support for embedded domain specific languages in \progLang{Haskell}. 

\item Specifically, we looked at \progLang{Prolog} in \progLang{Haskell}. 

\item Our ideas were tested by the means of some prototypes to find out that \progLang{Haskell} is an effective tool for embedding.  

\item Lastly, we contributed towards solving the programmer's dilemma of choosing programming languages.
\end{itemize}
